...
import { useSocket } from '@/lib/providres/socket-provider';
...
const QuillEditor: React.FC<QuillEditorProps> = ({
  ...
  useEffect(() => {
    if(socket === null || quill === null || !fileId || !localCursors.length) return
    const socketHandler = (range:any, roomId: string, cursorId: string) => {
      if(roomId === fileId){
        const cursorToMove = localCursors.find(
          (c:any) => c.cursors()?.[0] === cursorId
        )
        if(cursorToMove){
  ...
  return (
    <>
      ...
      <div className='flex justify-center items-center flex-col mt-2 relative'>
        <div className='w-full self-center maw-w-[800px] flex flex-col px-7 lg:my-8'>
          ...
          <div className='flex'>
            <BannerUpload
              ...
              id={fileId}
              dirType={dirType}
              className="mt-2 text-sm text-muted-foreground p-2 hover:text-card-foreground trasition-all rounded-md"
            >
            ...
            {details.bannerUrl && (
              <Button 
                ...
                variant="ghost" 
                className='gap-2 hover:bg-background flex items-center justify-center mt-2 text-sm w-36 text-muted-foreground p-2 rounded-md'
              > 
                <XCircleIcon size={16}/>
